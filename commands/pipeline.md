...
```
 ...
 ─────  ────────    ─────────────────────────────
  1     입력 필요    아이디어/자료 제공
  2     입력 필요    인터뷰 응답 (대규모 시 Codex CLI 자동 호출)
  3     자동         Codex CLI 자동 호출 (sandbox read-only + git-guard)
  4     자동         /agent-team-planner 실행
  5     자동         /multi-agent-review 실행
  6     자동         Codex CLI 자동 호출 (sandbox read-only + git-guard)
...
- `$ARGUMENTS`: 시작 지점 + 옵션
  ...
  - `--skip-external` → Codex 외부 리뷰(STAGE 3, 6, 11) 건너뛰기
    - 품질 게이트는 내부 리뷰로 대체:
      - STAGE 3 스킵 → STAGE 5(/multi-agent-review)의 CRITICAL 0건으로 충족
      - STAGE 6 스킵 → STAGE 5 결과가 이미 반영된 상태로 STAGE 7 진행
      - STAGE 11 스킵 → STAGE 10(/multi-agent-codereview)으로 충족 (Codex 코드 리뷰 생략)
...
║  │ STAGE 3: Codex 외부 리뷰 + 반영 (2-3회 루프)      │    ║
║  └─────────────────────────────────────────────────┘    ║
╠══════════════════════════════════════════════════════════╣
║  Phase B: 구현 계획 고도화                               ║
║  ┌─────────────────────────────────────────────────┐    ║
...
║  │ STAGE 6: Codex 외부 리뷰 + 반영 (2-3회 루프)      │    ║
║  │ STAGE 7: 최종 플랜 확정                          │    ║
║  └─────────────────────────────────────────────────┘    ║
╠══════════════════════════════════════════════════════════╣
║  Phase C: 구현 + 자동 개선                               ║
...
| 대규모 | 변경 파일 6개 이상 또는 신규 모듈 | **2b. Codex 초안 경로** |
...
상세 프로세스: `.claude/docs/plan_build_process_with_gem.md` (Gemini 시절 기록, deprecated. Codex 전환 내용은 이 섹션 + STAGE 3 참조)

1. Claude가 **Codex용 브리핑** 작성 → `.claude/prompts/{플랜명}-briefing.md`
   - 프로젝트 개요, 현재 코드 상태, 기술 스택
   - IMMUTABLE CONSTRAINTS (DO NOT 규칙 전문)
   - 이번 플랜의 목표/범위
2. Claude가 `Bash` 툴로 Codex CLI 자동 호출 → 초안 수신
   ```bash
   codex exec -s read-only --skip-git-repo-check --color never \
     -o .claude/prompts/{플랜명}-draft-raw.md \
     "$(cat .claude/prompts/{플랜명}-briefing.md)" \
     2> .claude/prompts/{플랜명}-draft-raw.stderr.log
   ```
   - `-s read-only` (Seatbelt 기반 OS-level 읽기 전용 샌드박스) + git-guard로 파일 수정 원천 차단
   - `-o <file>` 에이전트 최종 응답만 파일 저장 (stdout은 progress trace)
3. Claude가 정제:
...
## STAGE 3: Codex 외부 리뷰 (플랜 초안, 2-3회)

Codex CLI(GPT)에서 플랜 초안을 다중 유형 프롬프트로 교차검증합니다.
상세 프로세스: `.claude/docs/plan_build_process_with_gem.md` (Gemini 시절 기록, deprecated)
...
| 유형별 Codex 지시 | 범위 제한, 웹서치 요청 여부, 창의 범위 |
...
1. **자기완결형**: 공통 브리핑 + 개별 질문이 합쳐진 완성본. `codex exec "$(cat ...)"` 한 번 호출로 전달 가능
2. **1 프롬프트 = 1 파일**: `.claude/prompts/{플랜명}-{버전}/input/{Type}{번호}_{topic}.md`
3. **공통 브리핑 매번 포함**: "아까 보낸 거 참고해" 금지 — Codex 세션 간 컨텍스트 독립성 원칙에 맞춤
4. **IMMUTABLE CONSTRAINTS 전문 매번 삽입**: 축약/참조 금지
5. **각 프롬프트는 별도 `codex exec` 호출로 실행**: `--ephemeral` 또는 별개 invocation으로 세션 오염 차단
6. **산출물 형식 강제**: ID 채번 + 심각도/우선순위 + 요약 테이블 (Claude 파싱용)
7. **Type A는 범위 제한 명시**, 나머지 유형은 창의 범위를 열되 CONSTRAINTS만 울타리
8. **플랜 수정 금지 지시 필수**: 프롬프트 말미에 "원본 플랜 수정 금지. 리뷰 리포트만 `-o` 파일로 출력. 파일 생성/수정 금지." 강제 문구 삽입 — 외부 LLM이 "리뷰"를 "개선 작업"으로 해석해 파일을 수정하는 경향에 대한 이중 방어 (`-s read-only` + 프롬프트 강제 + git-guard의 3중 방어 중 2단계)
...
while round <= 3:
    1. Claude가 ABC 프롬프트 생성 → .claude/prompts/{플랜명}-{version}/input/
       ...
       - 각 프롬프트 말미에 "원본 플랜 수정 금지, 리뷰 리포트만 -o 파일로 출력" 강제 문구 삽입 (규칙 #8)

    2. Claude가 Bash 툴로 각 input 파일을 codex CLI로 자동 호출:
       ...
       - 호출:
           codex exec \
             ...
             --color never \
             --ephemeral \
             -o .claude/prompts/{플랜명}-{version}/output/{파일}.md \
             "$(cat .claude/prompts/{플랜명}-{version}/input/{파일}.md)" \
             2> .claude/prompts/{플랜명}-{version}/output/{파일}.stderr.log
         · `-s read-only` → Seatbelt 기반 OS-level 읽기 전용 샌드박스, 파일 수정 원천 차단
         · `--skip-git-repo-check` → 외부 리뷰이므로 repo 위치 체크 불필요
         · `--ephemeral` → 세션 파일 persist 안 함 (세션 간 격리 + 디스크 오염 방지)
         · `-o <file>` → 에이전트 최종 응답만 파일로 저장 (stdout은 progress trace)
...
외부 LLM(과거 Gemini, 현재 Codex)이 "리뷰 요청"을 "개선 작업"으로 오인해 파일을 무단 생성/수정할 수 있는 위험에 대비해 3중 방어를 적용한다:

| 계층 | 방어 수단 | 역할 |
|---|---|---|
| 1. CLI 샌드박스 | `codex exec -s read-only` | macOS Seatbelt 기반 OS-level 읽기 전용 샌드박스. kernel이 모든 파일쓰기를 차단하므로 LLM 의도와 무관하게 수정 불가 (이전 Gemini `--approval-mode plan` 대비 강화) |
| 2. 프롬프트 강제 | "수정 금지, 리포트는 `-o` 파일로만 출력" 문구 | LLM 의도 수준에서 행동 제약 |
| 3. git-guard | 호출 전후 `git status --porcelain` 비교 | 앞선 2단계를 우회한 수정을 외부에서 감지 + rollback |

어느 한 층이 실패해도 나머지 층에서 잡아낼 수 있도록 설계. 계층 1(샌드박스)은 Codex 전환 이후 가장 강력한 기본 방어선이 되었고, 2/3은 depth-in-defense 보조층.

### 디렉토리 구조

```
.claude/prompts/{플랜명}-{버전}/
├── input/                  ← Claude가 생성, `codex exec`로 자동 호출
│   ├── A1_{topic}.md
│   ├── A2_{topic}.md
│   ├── B1_{topic}.md
│   └── C1_{topic}.md
└── output/                 ← Codex가 `-o <file>`로 최종 응답 저장, Claude가 파싱
    ├── A1_{topic}.md
    ├── A1_{topic}.stderr.log
    └── B1_{topic}.md
```
...
## STAGE 6: Codex 외부 리뷰 (고도화된 플랜, 2-3회)
...
- Codex 프롬프트: `.claude/prompts/client-onboarding-v1.2/`
...
├── prompts/
│   ├── {플랜명}-briefing.md       ← STAGE 2b Codex 브리핑
