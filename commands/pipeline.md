...
```
 ...
 ─────  ────────    ─────────────────────────────
  1     입력 필요    아이디어/자료 제공
  2     입력 필요    인터뷰 응답 (대규모 시 codex:rescue 초안 위임)
  3     자동         codex:rescue A/B/C 직렬 3회 호출 (리뷰-only 모드)
  4     자동         /agent-team-planner 실행
  5     자동         /multi-agent-review 실행
  6     자동         codex:rescue A/B/C 직렬 3회 호출 (리뷰-only 모드)
...
- `$ARGUMENTS`: 시작 지점 + 옵션
  ...
  - `--skip-external` → 외부 리뷰(STAGE 3, 6, 11) 건너뛰기
    - 품질 게이트는 내부 리뷰로 대체:
...
| 대규모 | 변경 파일 6개 이상 또는 신규 모듈 | **2b. Codex rescue 초안 경로** |
...
### 2b. Codex rescue 초안 경로

상세 프로세스: `.claude/docs/plan_build_process.md`

1. Claude가 **Codex rescue용 브리핑** 작성 → `.claude/prompts/{플랜명}-briefing.md`
   - 프로젝트 개요, 현재 코드 상태, 기술 스택
   - IMMUTABLE CONSTRAINTS (DO NOT 규칙 전문)
   - 이번 플랜의 목표/범위
   - 출력 규약: "stdout 마크다운만, 파일 수정 금지"
2. Claude가 `codex:rescue` 서브에이전트에 브리핑을 전달하고 초안 마크다운을 받아 `.claude/prompts/{플랜명}-draft-raw.md`로 저장
   - rescue sandbox 내장으로 파일 수정 리스크 구조적으로 낮음
   - 응답에 수정 diff/패치 포함 시 해당 응답 전체 폐기 + 재요청
3. Claude가 정제:
...
Codex rescue(GPT-5.4)로 플랜 초안을 다중 유형 프롬프트로 교차검증합니다.
상세 프로세스: `.claude/docs/plan_build_process.md`
...
| 유형별 rescue 지시 | 범위 제한, 탐색 깊이, 창의 범위 |
...
1. **자기완결형**: 공통 브리핑 + 개별 질문이 합쳐진 완성본. 한 번의 `codex:rescue` 호출로 전달 가능
2. **1 프롬프트 = 1 파일**: `.claude/prompts/{플랜명}-{버전}/input/{Type}{번호}_{topic}.md`
3. **공통 브리핑 매번 포함**: "아까 보낸 거 참고해" 금지 — rescue 세션 간 컨텍스트 공유를 가정하지 말 것
4. **IMMUTABLE CONSTRAINTS 전문 매번 삽입**: 축약/참조 금지
5. **각 프롬프트는 별도 `codex:rescue` 직렬 호출로 실행**: **병렬 호출 금지** (rescue 런타임 상태 공유 가능성) — 호출 간 컨텍스트 격리로 세션 오염 차단
6. **산출물 형식 강제**: ID 채번 + 심각도/우선순위 + 요약 테이블 (Claude 파싱용)
7. **Type A는 범위 제한 명시**, 나머지 유형은 창의 범위를 열되 CONSTRAINTS만 울타리
8. **리뷰-only 강제 필수 (2중 방어)**:
   - 프롬프트 **첫 줄**에 `리뷰 모드: 리포트만 작성, 파일·플랜 수정 절대 금지` 명시
   - 프롬프트 **말미**에 `원본 플랜의 어떤 부분도 수정하지 마시오. 리뷰 의견만 ## Findings, ## Risks, ## Recommendations 섹션으로 작성하시오.` 강조
   - rescue 응답에 수정 diff/패치 제안 포함 시 해당 라운드 전체 폐기 후 재요청 (필터링 대상 아님)

### 루프 구조 (codex:rescue 직렬 자동화)
...
while round <= 3:
    1. Claude가 ABC 프롬프트 생성 → .claude/prompts/{플랜명}-{version}/input/
       ...
       - 각 프롬프트 첫 줄/말미에 "리뷰 모드: 리포트만 작성, 파일·플랜 수정 절대 금지" 문구 삽입 (규칙 #8)

    2. Claude가 codex:rescue 서브에이전트를 직렬로 호출 (A → B → C 순, 병렬 금지):
       - 각 호출에 input/{파일}.md 내용을 프롬프트로 전달
       - rescue 응답(stdout)을 output/{파일}.md로 저장
       - 다음 라운드 시작 전 이전 라운드 완료 대기
       - 응답 검증:
         · 응답이 ## Findings / ## Risks / ## Recommendations 형식 준수 → 정상, output 유지
         · 응답에 수정 diff/패치/"수정된 플랜" 포함 → 해당 라운드 전체 폐기 (rm output/*) → 프롬프트 강제 문구 재확인 후 재호출
         · 3회 재호출 실패 → 수동 개입 큐로 분리
       - rescue sandbox 내장으로 파일 무단 수정 리스크는 구조적으로 낮음 (이전 세대 git-guard 방어 불필요)
...
Codex rescue의 기본 동작은 "조사→진단→수정"이므로, 플랜 리뷰 용도로 쓸 때는 리뷰-only 모드로 강제해야 한다. rescue의 sandbox 격리 위에 프롬프트 강제 + 응답 검증의 다층 방어:

| 계층 | 방어 수단 | 역할 |
|---|---|---|
| 1. Sandbox | rescue 내장 sandbox | 파일 수정이 격리 환경에만 국한 (이전 세대 CLI의 `--approval-mode plan` 상위 대체) |
| 2. 프롬프트 강제 (첫 줄) | "리뷰 모드: 리포트만 작성, 파일·플랜 수정 절대 금지" | LLM 의도 수준에서 행동 제약 |
| 3. 프롬프트 강제 (말미) | "원본 플랜의 어떤 부분도 수정하지 마시오. ## Findings / ## Risks / ## Recommendations 섹션으로만 작성" | 응답 형식 강제 |
...
├── input/                  ← Claude가 생성, codex:rescue로 직렬 호출
│   ├── A1_{topic}.md
│   ├── A2_{topic}.md
│   ├── B1_{topic}.md
│   └── C1_{topic}.md
└── output/                 ← Claude가 rescue 응답을 자동 저장 후 파싱
    ├── A1_{topic}.md
    └── B1_{topic}.md
```
...
- Codex rescue 프롬프트: `.claude/prompts/client-onboarding-v1.2/`
...
├── prompts/
│   ├── {플랜명}-briefing.md       ← STAGE 2b Codex rescue 브리핑
