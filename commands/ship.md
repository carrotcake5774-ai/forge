...
- **Evidence 수집 — 한 source 만 보지 말고 가능한 만큼 모은다.** 추측 fix 로 직행하기 전에:
  - `gh run view {run_id} --log-failed` — 실패 step 로그 (가장 빠른 진단)
  - `gh run view {run_id} --log` — 전체 로그 (실패 직전 컨텍스트 포함)
  - `gh api repos/{owner}/{repo}/actions/runs/{run_id}/jobs` — job 별 timing, runner OS/arch, matrix leg 정보 (annotation 은 이 endpoint 에 없음)
  - 각 job 의 annotation 이 필요하면 `gh api repos/{owner}/{repo}/check-runs/{check_run_id}/annotations` (별도 endpoint, check_run_id 는 `/commits/{ref}/check-runs` 에서 resolve)
  - artifact 가 있다고 기대되면 `gh run download {run_id}` 시도 — **non-zero exit 시 무시하고 나머지 source 로 계속**. download 실패는 진행을 막지 않는다
- evidence 가 *어느 layer 에서* 깨지는지 가리킬 때까지 분석 (lint? test? collection? 인프라?)
- 실패 원인 분류 (evidence 기반):
  - **lint/format**: 자동 수정 → 새 커밋 → 재푸시
  - **test 실패**: 원인 분석 → 코드 수정 → 새 커밋 → 재푸시
  - **collection/import 에러**: 원인 분석 우선 (출력 텍스트만 보면 test 실패와 혼동)
